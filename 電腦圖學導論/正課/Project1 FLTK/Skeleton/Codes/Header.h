...
private:
	...
	glm::vec4 resample3X3(int u, int v);
	glm::vec4 resample4X4(int u, int v);
	glm::vec4 resample3X4(int u, int v);
	glm::vec4 resample4X3(int u, int v);

	enum _CompImage { _over, _in, _out, _atop, _xor };
	void compImage(TargaImage* img, int method);
...
// reverse the rows of the image, some targas are stored bottom to top
	...
	void NPR_Paint_Layer(glm::vec4* tCanvas, const glm::vec4* tReferenceImage, int tBrushSize);
