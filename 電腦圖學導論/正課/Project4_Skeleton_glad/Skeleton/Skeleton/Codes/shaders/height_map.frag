...
vec3 getWallColor(vec3 point) {
  ...
  vec3 getSurfaceRayColor(vec3 origin, vec3 ray, vec3 waterColor) {
        vec3 color;
        if (ray.y < 0.0) {
		    vec2 t = intersectCube(origin, ray, vec3(-1.0, -poolHeight, -1.0), vec3(1.0, 1.0, 1.0));
...
void main()
{
    ...
    vec3 RefractColor = getSurfaceRayColor(vec3(f_in.texture_coordinate.x*2-1,0.0f,f_in.texture_coordinate.y*2-1), RefractVec_up, vec3(1.0)) * vec3(0.8, 1.0, 1.1);

		//vec3
		//vec3 origin = f_in.position;
...
//    }
   ...
   // else if(reflect_open)f_color = vec4(ReflectColor, 1.0);
    ...
    //f_color = texture2D(tiles,f_in.texture_coordinate);
